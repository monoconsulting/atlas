...
## Local (host-bound to 127.0.0.1)
- Frontend (Nginx): `http://127.0.0.1:${FRONTEND_PORT}`
- Core API (HTTP):  `http://127.0.0.1:${CORE_API_PORT}`
- Price Engine:     `http://127.0.0.1:${PRICE_ENGINE_PORT}`
- Postgres:         `127.0.0.1:${POSTGRES_PORT}` (TCP)

## Intra-container (Docker DNS)
- core-api:      `http://core-api:8000`
- price-engine:  `http://price-engine:8001`
- postgres:      `postgres:5432`

## Base Paths & Versioning
- Core API base path: `/v1/`
- Price Engine base:  `/v1/`
- Version policy: **path-based** (`/v1` → `/v2`). Deprecations announced ≥90 days in advance.
